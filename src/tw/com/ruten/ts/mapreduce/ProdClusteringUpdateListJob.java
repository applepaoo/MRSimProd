...
public class ProdClusteringUpdateListJob extends Configured implements Tool {
    ...
    public static class UpdateListJobNewMapper extends Mapper<Object, Text, Text, MapWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (clusterField != null && clusterField.size() == data.length) {
                ...
                outValue.put(new Text("IS_OLD"), new Text("N"));
                context.write(outKey, outValue);
            }
        }
    }

    public static class UpdateListJobOldMapper extends Mapper<Object, Text, Text, MapWritable> {
        private Configuration conf;
        private Text outKey = new Text();
        private List<String> clusterField;
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (clusterField != null && clusterField.size() == data.length) {
                ...
                outValue.put(new Text("GROUP_NUM"), new Text(data[clusterField.indexOf("GROUP_NUM")]));
                outValue.put(new Text("UPDATE"), new Text(data[clusterField.indexOf("UPDATE")]));
                outValue.put(new Text("IS_OLD"), new Text("Y"));
                context.write(outKey, outValue);
            }
        }
    }
    ...
    public static class ProdClusteringUpdateListJobReducer extends Reducer<Text, MapWritable, Text, Text> {
        ...
        public void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
            ...
            Text currentIsOld = (Text) current.get(new Text("IS_OLD"));
            ...
            try {
                if (list.size() > 1) {
                    ...
                    if (current_group_num > 1 && last_group_num > 1 && (!lastFingerprint.toString().equalsIgnoreCase(currentFingerprint.toString()) || (current_group_head ^ last_group_head))) { //集群大於1 & (群集更動 | head 更動)
                        result.set(currentFingerprint.toString() + "\t" + current_group_head + "\t" + current_group_num + "\t" + currentIsOld.toString() + "\tupdate");
                        if (isWrite2Solr) {
                    ...
                    } else if (last_group_num <= 1 && current_group_num > 1) {// 變集群大於1
                        result.set(currentFingerprint.toString() + "\t" + current_group_head + "\t" + current_group_num + "\t" + currentIsOld.toString() + "\tinsert");
                        if (isWrite2Solr) {
                            groupTagUpdateModels.add((JSONObject) jsonParser.parse(gson.toJson(new GroupTagUpdateModel(key.toString(), currentFingerprint.toString(), current_group_head, current_group_num))));
                        }
                    } else if (last_group_num > 1 && current_group_num <= 1) {// 變集群小於1
                        result.set(currentFingerprint.toString() + "\t" + current_group_head + "\t" + current_group_num + "\t" + currentIsOld.toString() + "\tdelete");
                        if (isWrite2Solr) {
                            groupTagUpdateModels.add((JSONObject) jsonParser.parse(gson.toJson(new GroupTagUpdateModel(key.toString(), null, null, null))));
                        }
                    }
                } else {
                    if (currentIsOld.toString().equals("N") && current_group_num > 1) { // 變上架且集群大於1
                        result.set(currentFingerprint.toString() + "\t" + current_group_head + "\t" + current_group_num + "\t" + currentIsOld.toString() + "\tinsert");
                        if (isWrite2Solr) {
                            groupTagUpdateModels.add((JSONObject) jsonParser.parse(gson.toJson(new GroupTagUpdateModel(key.toString(), currentFingerprint.toString(), current_group_head, current_group_num))));
                        }
                    } else if (currentIsOld.toString().equals("Y")) {//變下架
                        result.set(currentFingerprint.toString() + "\t" + current_group_head + "\t" + current_group_num + "\t" + currentIsOld.toString() + "\tdelete");
                        if (isWrite2Solr) {
                            groupTagUpdateModels.add((JSONObject) jsonParser.parse(gson.toJson(new GroupTagUpdateModel(key.toString(), null, null, null))));
                        }
    ...
    public int run(String[] args) throws Exception {
        ...
        Path currentFilePath = new Path(args[0]);
...
//        job.setNumReduceTasks(37);
        ...
        MultipleInputs.addInputPath(job, currentFilePath, TextInputFormat.class, UpdateListJobNewMapper.class);
        if (oldFilePath != null) {
            MultipleInputs.addInputPath(job, oldFilePath, TextInputFormat.class, UpdateListJobOldMapper.class);
        }
